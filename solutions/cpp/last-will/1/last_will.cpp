...
namespace estate_executor {
    int assemble_account_number(int secret_modifier) {
        int assembled_acount_number = garcia::bank_number_part(secret_modifier) + khan::bank_number_part(secret_modifier) + zhang::bank_number_part(secret_modifier);
        
        return assembled_acount_number;
    }
    int assemble_code(){
        //Sum the blue, then the red fragments, then multiplying both.
