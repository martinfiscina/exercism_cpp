...
int monthly_rate(double hourly_rate, double discount) {
    ...
    // discount.
    float daily_pay = daily_rate(hourly_rate);
    float total_w_discount_day = apply_discount(daily_pay, discount);
    float total_monthly = total_w_discount_day * 22;
    return std::ceil(total_monthly);
}
...
int days_in_budget(int budget, double hourly_rate, double discount) {
    ...
    float total_w_discount_day = apply_discount(hourly_rate, discount);
    float total_hours = budget / total_w_discount_day;
    float days = total_hours / 8;
